...
//log_printf(15, "handle_allocate: create time =" TT " ns=%d\n", a.creation_ts.time, ns_getid(task->ns));
   ...
   snprintf(token, sizeof(token), "%d ibp://%s:%d/%s#%s/" LU "/READ "
       ...
       IBP_OK,
       nm_get_host(nm), nm_get_port(nm), res->name, a.caps[READ_CAP].v, cid,
       nm_get_host(nm), nm_get_port(nm), res->name, a.caps[WRITE_CAP].v, cid,
       nm_get_host(nm), nm_get_port(nm), res->name, a.caps[MANAGE_CAP].v, cid);
...
//log_printf(10, "handle_internal_get_alloc: ns=%d i=%d\n", ns_getid(task->ns), i);
         ...
         bytes_used = blen;
         state = good_block;
         convert_bin2hex(bin_len, (unsigned char *)block_chksum, hex_digest);
         if (state == 0) {
...
int handle_status(ibp_task_t *task)
{
  ...
  if (status->subcmd == IBP_ST_VERSION) {
     ...
     snprintf(buffer, sizeof(buffer)-1, "Pending RID count: %d\n", stack_size(global_config->rl->pending));
     strncat(result, buffer, sizeof(result)-1 - strlen(result));
     if (stack_size(global_config->rl->pending) > 0) {
        snprintf(buffer, sizeof(buffer)-1, "Pending RID list: ");
        strncat(result, buffer, sizeof(result)-1 - strlen(result));
        move_to_top(global_config->rl->pending);
        while ((prid = (char *)get_ele_data(global_config->rl->pending)) != NULL) {
          move_down(global_config->rl->pending);
          if (get_ele_data(global_config->rl->pending) == NULL) {
...
//  }
  ...
  if (a->type == IBP_BYTEARRAY) {
     ...
     if ((task->enable_chksum == 1) && (err == 1)) {
        err = ns_read_chksum_flush(task->ns);
        ns_read_chksum_disable(task->ns);
        if (err != 0) {
...
log_printf(10, "handle_read: mid: " LU " offset=" I64T " len=" I64T " ns=%d\n", id, alias_offset, alias_len, ns_getid(task->ns));
  ...
  if (a->type == IBP_BYTEARRAY) {
     if (task->enable_chksum == 1) { ns_write_chksum_set(task->ns, task->ncs); ns_write_chksum_enable(task->ns); }
     err = 0;
     nleft = r->iovec.total_len;
     r->iovec.transfer_total = 0;

     while (err == 0) {
        log_printf(20, "handle_read: a->r_pos=" LU " left=" LU " ns=%d\n", a->r_pos, nleft, task->ns->id);
    ...
    //** Validate the partial block if needed
     if ((task->enable_chksum == 1) && (err == 1)) {
        err = ns_write_chksum_flush(task->ns);
        ns_write_chksum_disable(task->ns);
        if (err != 0) {
...
log_printf(15, "iovec->n=%d off=" OT " len=" OT " cumulative_len=" OT "\n", iov->n, iov->vec[0].off, iov->vec[0].len, iov->vec[0].cumulative_len);
  ...
  switch (r->transfer_dir) {
    ...
    case IBP_PUSH_CHKSUM:
       if (r->write_mode == 1) {
          snprintf(write_cmd, 1024, "%d %d %d " I64T " %s %s " OT " %d \n", IBPv040, IBP_STORE_CHKSUM, ns_chksum_type(&(task->ncs)), ns_chksum_blocksize(&(task->ncs)), key, typekey, rlen, rto);
       } else {
          snprintf(write_cmd, 1024, "%d %d %d " I64T " %s %s " OT " " OT " %d \n", IBPv040, IBP_WRITE_CHKSUM, ns_chksum_type(&(task->ncs)), ns_chksum_blocksize(&(task->ncs)),key, typekey, r->remote_offset, rlen, rto);
       }
       break;
    case IBP_PULL:
    ...
    case IBP_PULL_CHKSUM:
       snprintf(write_cmd, 1024, "%d %d %d " I64T " %s %s " OT " " OT " %d \n", IBPv040, IBP_LOAD_CHKSUM, ns_chksum_type(&(task->ncs)), ns_chksum_blocksize(&(task->ncs)), key, typekey, r->remote_offset, rlen, rto);
       break;
  }
  log_printf(15, "handle_transfer: ns=%d sending command: %s", task->ns->id, write_cmd);
  write_cmd[1023] = '\0';

  err = write_netstream_block(ns, task->cmd_timeout, write_cmd, strlen(write_cmd));
  if (err != NS_OK) {
     if (task->cmd_timeout > apr_time_now()) {
        log_printf(10, "handle_transfer: TIMEOUT Error sending command! write_netstream_block=%d  cmd=%s\n", err, write_cmd);
        send_cmd_result(task, IBP_E_SERVER_TIMEOUT);
     } else {
        log_printf(10, "handle_transfer: Error sending command! write_netstream_block=%d  cmd=%s\n", err, write_cmd);
        send_cmd_result(task, IBP_E_CONNECTION);
     }
     return(0);
  }
...
//asm volatile ("int3;");
  ...
  if ((task->enable_chksum == 1) && (err == 1)) {
     if (r->transfer_dir == IBP_PUSH) {
        err = ns_write_chksum_flush(ns);
        ns_write_chksum_disable(ns);
     } else {
        err = ns_read_chksum_flush(ns);
  ...
  if (err == -1) {  //** Dead connection
     ...
     result[511] = '\0';
     log_printf(10, "handle_transfer: ns=%d Completed.  Sending result: %s", task->ns->id, result);
     write_netstream_block(task->ns, task->cmd_timeout, result, strlen(result));
...
int handle_internal_get_config(ibp_task_t *task)
{
  ...
  err = write_netstream_block(task->ns, task->cmd_timeout, result, strlen(result));

  //** and the config
  err = write_netstream_block(task->ns, task->cmd_timeout, buffer, used);
...
int handle_internal_mount(ibp_task_t *task)
{
  ...
  //** Find the group
  sgrp = NULL;
  igrp = inip_first_group(keyfile);
  found = 0;
  for (igrp = inip_first_group(keyfile); igrp != NULL; igrp = inip_next_group(igrp)) {
     sgrp = inip_get_group(igrp);
     if (strncmp("resource", sgrp, 8) == 0) {
        str = strdup(sgrp);
        string_token(str, " ", &bstate, &fin);  //** This retreives the "resource"
        srid = string_token(NULL, " ", &bstate, &fin);  //** This should be the RID
        string_token(NULL, " ", &bstate, &fin);
...
int handle_internal_umount(ibp_task_t *task)
{
  ...
  while (dt < arg->delay) {
    ...
    dt = apr_time_sec(dt);
    log_printf(5, "dt=%d delay=%d count=%d last_count=%d\n", dt, arg->delay, count, last_count);
  }
