...
int _fs_calculate_chksum(osd_fs_t *fs, osd_fs_fd_t *fsfd, chksum_t *cs, osd_off_t block, char *buffer, osd_off_t bufsize, char *calc_chksum, int correct_errors);
osd_off_t _chksum_buffered_read(osd_fs_t *fs, osd_fs_fd_t *fsfd, osd_off_t len, char *buffer, osd_off_t bufsize, chksum_t *cs1, chksum_t *cs2);
...
fs_cache_table_t *fs_cache_create(apr_pool_t *pool, int n_blocks, int block_size, apr_time_t max_wait)
{
  ...
  type_malloc_clear(c, fs_cache_table_t, 1);
  ...
  type_malloc_clear(c->block_free_count, int, n_blocks);
  type_malloc_clear(c->block_last_offset, int, n_blocks);
  for (i=0; i<n_blocks; i++) c->block_free_count[i] = block_size;

  type_malloc_clear(c->table, fs_cache_entry_t, c->n_total+1);
...
{
  return(release_pigeon_coop_hole(fsfd->my_range_coop, &(fsfd->my_range_slot)));
}
...
int insert_range(pigeon_coop_t *coop, osd_fs_fd_t *fsfd, int rtype, int64_t timestamp, osd_off_t start_block, osd_off_t end_block)
{
  osd_fs_range_t *r;

  fsfd->my_range_coop = coop;
  fsfd->my_range_slot = reserve_pigeon_coop_hole(coop);
  r = pigeon_coop_hole_data(&(fsfd->my_range_slot));
...
int check_range_overlap(pigeon_coop_t *coop, int64_t timestamp, osd_off_t start_block, osd_off_t *end_block)
{
   pigeon_coop_iter_t it;
   pigeon_coop_hole_t i;
   osd_fs_range_t *r;

   it = pigeon_coop_iterator_init(coop);
...
void _range_request_wakeup(pigeon_coop_t *coop, osd_off_t lo, osd_off_t hi)
{
   pigeon_coop_iter_t it;
   pigeon_coop_hole_t i;
   osd_fs_range_t *r;
...
log_printf(15,"fs_create_id: id=" LU " chksum_type=%d\n", id, chksum_type);
   ...
   if (chksum_valid_type(chksum_type) == 1) {
      ...
      chksum_set(&chksum, chksum_type);
      chksum_get(&chksum, CHKSUM_DIGEST_BIN, cs_value);
      dbytes = 0;
      cs_size = chksum_size(&chksum, CHKSUM_DIGEST_BIN);
...
osd_id_t fs_trash_undelete(osd_t *d, int trash_type, const char *trash_id) {
   ...
   string_token(wname, "_", &bstate, &n);  //** Throw away the time
//   sscanf(string_token(NULL, "_", &bstate, &n), LU, &id);
char *dummy = string_token(NULL, "_", &bstate, &n);
...
int fs_truncate(osd_t *d, osd_id_t id, osd_off_t l_size) {
   ...
   fcs = &(fsfd->obj->fd_chksum);
   if (chksum_type(&(fcs->chksum)) != CHKSUM_NONE) {
      cs = fcs->chksum;
...
osd_fs_object_t *fs_object_get(osd_fs_t *fs, osd_id_t id)
{
  ...
  if (obj == NULL) {  //** New object
     pch = reserve_pigeon_coop_hole(fs->obj_list);
     obj = (osd_fs_object_t *)pigeon_coop_hole_data(&pch);
     obj->id = id;
     obj->my_slot = pch;
...
osd_off_t _chksum_buffered_read(osd_fs_t *fs, osd_fs_fd_t *fsfd, osd_off_t len, char *buffer, osd_off_t bufsize, chksum_t *cs1, chksum_t *cs2)
{
  ...
  for (i = 0; i < nblocks; i++) {
     ...
     chksum_add(cs1, bufsize, buffer);
     if (cs2 != NULL) chksum_add(cs2, bufsize, buffer);
  }
...
//ftello(fsfd->fd);
     ...
     chksum_add(cs1, nleft, buffer);
     if (cs2 != NULL) chksum_add(cs2, nleft, buffer);
  }
...
//  char dummy_value[CHKSUM_MAX_SIZE+2];
  ...
  if ((herr == 0) && (block_bytes_used > 0)) {  //** If it's a valid with data
    chksum_reset(cs);  //** Reset the chksum
    d = block_bytes_used;
...
//start_time = apr_time_now();
  ...
  //** Prepare the chksum **
  chksum_reset(cs);
  chksum_add(cs, ocs->blocksize, buffer);
  chksum_get(cs, CHKSUM_DIGEST_BIN, cs_value);
...
chksum_get(cs, CHKSUM_DIGEST_HEX, dummy_value);
...
osd_off_t chksum_direct_read(osd_fs_t *fs, osd_fs_fd_t *fsfd, osd_off_t block, osd_off_t offset, osd_off_t len, buffer_t data_ptr) 
{
  ...
  obj_offset += offset + chksum_size(&(fsfd->chksum), CHKSUM_DIGEST_BIN) + sizeof(uint32_t);
...
log_printf(15, "fs_chksum_info: after open id=" LU "\n", id); flush_log();
   ...
   *blocksize = 0;
   ...
   if (fcs->is_valid == 1) {  //** No chksum available so return
     *cs_type = chksum_type(&(fcs->chksum));
...
//log_printf(0, "fs_get_chksum: 00000000000000000\n"); flush_log();
   ...
   cs_size = chksum_size(&(fcs->chksum), CHKSUM_DIGEST_BIN);
...
//convert_bin2hex(cs_size, (unsigned char *)disk_value, hex_buffer);
...
//convert_bin2hex(cs_size, (unsigned char *)calc_value, hex_buffer);
...
int _fs_calculate_chksum(osd_fs_t *fs, osd_fs_fd_t *fsfd, chksum_t *cs, osd_off_t block,
    char *buffer, osd_off_t bufsize, char *calc_value, int correct_errors)
{
  ...
  n = 0;
  n = fread(&block_bytes_used, 1, sizeof(uint32_t), fsfd->fd);
  cs_size = chksum_size(cs, CHKSUM_DIGEST_BIN);
  ...
  for (nleft = 0; nleft < nblocks; nleft++) {
     n = n + fread(buffer, 1, bufsize, fsfd->fd);
     chksum_add(cs, bufsize, buffer);
  }
  nleft = block_bytes_used % bufsize;
  if (nleft > 0) {
     n = n + fread(buffer, 1, nleft, fsfd->fd);
     chksum_add(cs, nleft, buffer);
  }
...
log_printf(15, "fs_shelf_object_new: shelf=%p size=%d\n", shelf, size); 
  ...
  for (i=0; i<size; i++) {
     ...
     obj->read_range_list = new_pigeon_coop("read_range", FS_RANGE_COUNT, sizeof(osd_fs_range_t), NULL, fs_shelf_range_new, fs_shelf_range_free);
     obj->write_range_list = new_pigeon_coop("write_range", FS_RANGE_COUNT, sizeof(osd_fs_range_t), NULL, fs_shelf_range_new, fs_shelf_range_free);
...
log_printf(15, "fs_shelf_object_free: shelf=%p size=%d\n", shelf, size); 
  for (i=0; i<size; i++) {
     ...
     destroy_pigeon_coop(obj->read_range_list);
     destroy_pigeon_coop(obj->write_range_list);
...
//#endif
   ...
   fs->obj_list = new_pigeon_coop("obj_list", FS_OBJ_COUNT, sizeof(osd_fs_object_t), (void *)fs, fs_shelf_object_new, fs_shelf_object_free); assert(fs->obj_list != NULL);
   fs->fd_list = new_pigeon_coop("fd_list", FS_OBJ_COUNT, sizeof(osd_fs_fd_t), (void *)fs, fs_shelf_fd_new, fs_shelf_fd_free); assert(fs->fd_list != NULL);
