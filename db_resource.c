...
int mount_db_generic(inip_file_t *kf, const char *kgroup, DB_env_t *env, DB_resource_t *dbres, int wipe_clean)
{
   char fname[2048];
   u_int32_t flags, bflags, psize;
   int err;
   ...
   assert((dbres->loc = inip_get_string(kf, kgroup, "loc", NULL)) != NULL);
...
int mount_db(inip_file_t *kf, const char *kgroup, DB_env_t *dbenv, DB_resource_t *dbres)
{
