...
int print_config(char *buffer, int *used, int nbytes, Config_t *cfg) 
{
  ...
  append_printf(buffer, used, nbytes, "[server]\n");
  append_printf(buffer, used, nbytes, "interfaces=");
  for (d=0; d<server->n_iface; d++) {
      iface = &(server->iface[d]);
      append_printf(buffer, used, nbytes, "%s:%d;", iface->hostname, iface->port);
  }
  append_printf(buffer, used, nbytes, "\n");

  append_printf(buffer, used, nbytes, "threads = %d\n", server->max_threads);
  append_printf(buffer, used, nbytes, "max_pending = %d\n", server->max_pending);
...
//log_printf(0, "print_timeout: s=%d ms=%d\n",d, k); 
  ...
  append_printf(buffer, used, nbytes, "splice_enable = %d\n", server->splice_enable);
  append_printf(buffer, used, nbytes, "db_env_loc = %s\n", cfg->dbenv_loc);
  append_printf(buffer, used, nbytes, "db_mem = %d\n", cfg->db_mem);
  append_printf(buffer, used, nbytes, "log_file = %s\n", server->logfile);
  append_printf(buffer, used, nbytes, "log_level = %d\n", server->log_level);
  append_printf(buffer, used, nbytes, "return_cap_id = %d\n", server->return_cap_id);
  append_printf(buffer, used, nbytes, "backoff_scale = %lf\n", server->backoff_scale);
  append_printf(buffer, used, nbytes, "backoff_max = %d\n", server->backoff_max);
  d = server->log_maxsize / 1024 / 1024;
  append_printf(buffer, used, nbytes, "log_maxsize = %d\n", d);
  append_printf(buffer, used, nbytes, "debug_level = %d\n", server->debug_level);
  append_printf(buffer, used, nbytes, "timestamp_interval = %d\n", server->timestamp_interval);
  append_printf(buffer, used, nbytes, "activity_file = %s\n", server->alog_name);
  d = server->alog_max_size / 1024 / 1024;
  append_printf(buffer, used, nbytes, "activity_maxsize = %d\n", d);
  append_printf(buffer, used, nbytes, "activity_max_history = %d\n", server->alog_max_history);
  append_printf(buffer, used, nbytes, "activity_host = %s\n", server->alog_host);
  append_printf(buffer, used, nbytes, "activity_port = %d\n", server->alog_port);
  append_printf(buffer, used, nbytes, "\n");
  ...
  append_printf(buffer, used, nbytes, "# Total Resources : %d\n\n", i);
...
void *worker_task(apr_thread_t *ath, void *arg)
{
   ...
   int ncommands;
   apr_status_t retval;
   Net_timeout_t start_read, start_handle, end_time, dt_read, dt_handle, dt_total;

   log_printf(10, "worker_task: ns=%d ***START*** Got a connection at " TT "\n", th->ns->id, apr_time_now());
...
//Net_timeout_t dt;
//set_net_timeout(&dt,1,0);
//readline_netstream_raw(task.ns, buffer, sizeof(buffer), dt, &status);

      reject_task(task.ns, 0);     //** Reject the connections
      close_netstream(task.ns);  //** And close it

      //** Push myself on the completed task
      apr_thread_mutex_lock(taskmgr.lock);
      push(taskmgr.completed, th);
   ...
   while ((shutdown_request() == 0) && (closed == 0)) {
      ns_read_chksum_clear(task.ns);
      ns_write_chksum_clear(task.ns);
...
void spawn_new_task(NetStream_t *ns, int reject_connection)
{
...
void reject_task(NetStream_t *ns, int destroy_ns)
{
  ...
  close_netstream(ns);
  if (destroy_ns == 1) destroy_netstream(ns);
}
