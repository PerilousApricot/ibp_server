...
int parse_chksum(ibp_task_t *task, char **bstate)
{
   ...
   chksum_set(&cs, type);
   return(ns_chksum_set(&(task->ncs), &cs, bsize));
}
...
int read_allocate(ibp_task_t *task, char **bstate)
{
   ...
   if (cmd->version > IBPv031) {
      if ((cmd->command == IBP_ALLOCATE_CHKSUM) || (cmd->command == IBP_SPLIT_ALLOCATE_CHKSUM)) {
         ...
         ca->cs_type = d;
         if ((chksum_valid_type(d) == 0) && (d!=CHKSUM_NONE)) {
            log_printf(10, "read_allocate: bad chksum_type=%d\n", d);
            send_cmd_result(task, IBP_E_CHKSUM_TYPE);
            return(-1);
         }

         len = 0; sscanf(string_token(NULL, " ", bstate, &fin),  I64T, &len);
         ca->cs_blocksize = len;
...
int read_validate_get_chksum(ibp_task_t *task, char **bstate)
{
   ...
   i=0; sscanf(string_token(NULL, " ", bstate, &finished), "%d", &i);
...
//    version IBP_VEC_WRITE_CHKSUM chksum_type block_size key typekey n_ele offset_1 len_1 offset_2 len_2 ... offset_N len_N timeout \n
//
//*****************************************************************
...
int read_write(ibp_task_t *task, char **bstate)
{
   ...
   if ((cmd->command == IBP_VEC_WRITE_CHKSUM) || (cmd->command == IBP_VEC_WRITE)) {
      llu = 0; sscanf(string_token(NULL, " ", bstate, &finished), "%llu", &llu);  //** Get the count
      if (llu < 0) {
...
//
// IBP_VEC_READ_CHKSUM
//    version IBP_VEC_READ_CHKSUM chksum_type blocksize key typekey n_ele offset_1 len_1 offset_2 len_2 ... offset_N len_N timeout \n
//
// IBP_PHOEBUS_SEND
...
int read_internal_undelete(ibp_task_t *task, char **bstate)
{
   ...
   //*** Get the trash_type ***
   arg->trash_type = 0;
   sscanf(string_token(NULL, " ", bstate, &fin), "%d", &(arg->trash_type));
   ...
   //*** Get the duration ***
   sscanf(string_token(NULL, " ", bstate, &fin), LU, &duration);
