...
int parse_resource(Resource_t *res, inip_file_t *keyfile, char *group)
{
   char *str, *str2, *bstate;
   int i, fin;

   res->keygroup = strdup(group);
   res->name = inip_get_string(keyfile, group, "rid", NULL);
   if (res->name == NULL) {
   ...
   res->preallocate = inip_get_integer(keyfile, group, "preallocate", 0);
   res->update_alloc = inip_get_integer(keyfile, group, "update_alloc", 1);
   res->enable_write_history = inip_get_integer(keyfile, group, "enable_write_history", 1);
   res->enable_read_history = inip_get_integer(keyfile, group, "enable_read_history", 1);
   res->enable_manage_history = inip_get_integer(keyfile, group, "enable_manage_history", 1);
   res->enable_alias_history = inip_get_integer(keyfile, group, "enable_alias_history", 1);

   res->rescan_interval = inip_get_integer(keyfile, group, "rescan_interval", 86400);
   res->cleanup_interval = inip_get_integer(keyfile, group, "cleanup_interval", 500);

   res->trash_grace_period[RES_DELETE_INDEX] = inip_get_integer(keyfile, group, "delete_grace_period", 3600);
   res->trash_grace_period[RES_EXPIRE_INDEX] = inip_get_integer(keyfile, group, "expire_grace_period", 7*24*3600);
   res->preexpire_grace_period = inip_get_integer(keyfile, group, "preexpire_grace_period", 24*3600);

   res->max_duration = inip_get_integer(keyfile, group, "max_duration", 0);
   ...
   res->device = inip_get_string(keyfile, group, "device", NULL);
   if (res->device == NULL) {
      printf("parse_resource: (%s) Missing resource device\n",group);
      abort();
   }

   str = inip_get_string(keyfile, group, "max_size", NULL);
   if (str == NULL) {
   ...
   str = inip_get_string(keyfile, group, "soft_size", NULL);
   if (str == NULL) {
   ...
   str = inip_get_string(keyfile, group, "hard_size", NULL);
   if (str == NULL) {
   ...
   str = inip_get_string(keyfile, group, "minfree_size", NULL);
   if (str == NULL) {
   ...
   res->enable_chksum = inip_get_integer(keyfile, group, "enable_chksum", 0);
   res->chksum_blocksize = inip_get_integer(keyfile, group, "chksum_blocksize_kb", 64);
   ...
   str = inip_get_string(keyfile, group, "chksum_type", "SHA256");
   i = chksum_name_type(str);
   if (i != -1) {
      chksum_set(&(res->chksum), i);
   ...
   res->n_cache = inip_get_integer(keyfile, group, "n_cache", 100000);
   res->cache_expire = inip_get_integer(keyfile, group, "cache_expire", 30);
   res->cache_expire = apr_time_from_sec(res->cache_expire);

   //** Get the rwm_mode
   str = inip_get_string(keyfile, group, "mode", "read,write,manage");
   str2 = string_token(str, " ,:|", &bstate, &fin);
   res->rwm_mode = 0;
   fin = 0;
   while (fin == 0) {
      ...
      str2 = string_token(NULL, " ,:|", &bstate, &fin);
   }
   free(str);
...
int mount_resource(Resource_t *res, inip_file_t *keyfile, char *group, DB_env_t *dbenv,
...
int print_resource(char *buffer, int *used, int nbytes, Resource_t *res)
{
   ...
   append_printf(buffer, used, nbytes, "rid = %s\n", res->name);
   append_printf(buffer, used, nbytes, "max_duration = %d\n", res->max_duration);
   append_printf(buffer, used, nbytes, "resource_type = %s\n", res->device_type);
   append_printf(buffer, used, nbytes, "device = %s\n", res->device);
//   append_printf(buffer, used, nbytes, "db_location = %s\n", res->location);
   append_printf(buffer, used, nbytes, "update_alloc = %d\n", res->update_alloc);
   ...
   append_printf(buffer, used, nbytes, "enable_read_history = %d\n", res->enable_read_history);
   append_printf(buffer, used, nbytes, "enable_write_history = %d\n", res->enable_write_history);
   append_printf(buffer, used, nbytes, "enable_manage_history = %d\n", res->enable_manage_history);
   append_printf(buffer, used, nbytes, "enable_alias_history = %d\n", res->enable_alias_history);
   append_printf(buffer, used, nbytes, "cleanup_interval = %d\n", res->cleanup_interval);
   append_printf(buffer, used, nbytes, "rescan_interval = %d\n", res->rescan_interval);
   append_printf(buffer, used, nbytes, "delete_grace_period = %d\n", res->trash_grace_period[RES_DELETE_INDEX]);
   append_printf(buffer, used, nbytes, "expire_grace_period = %d\n", res->trash_grace_period[RES_EXPIRE_INDEX]);
   append_printf(buffer, used, nbytes, "preexpire_grace_period = %d\n", res->preexpire_grace_period);

   n = res->max_size[ALLOC_TOTAL]/1024/1024; append_printf(buffer, used, nbytes, "max_size = " I64T "\n", n);
   n = res->max_size[ALLOC_SOFT]/1024/1024; append_printf(buffer, used, nbytes, "soft_size = " I64T "\n", n);
   n = res->max_size[ALLOC_HARD]/1024/1024; append_printf(buffer, used, nbytes, "hard_size = " I64T "\n", n);
   n = res->minfree/1024/1024; append_printf(buffer, used, nbytes, "minfree_size = " I64T "\n", n);
   append_printf(buffer, used, nbytes, "preallocate = %d\n", res->preallocate);

   append_printf(buffer, used, nbytes, "enable_chksum = %d\n", res->enable_chksum);
   append_printf(buffer, used, nbytes, "chksum_type = %s\n", chksum_name(&(res->chksum)));
   n = res->chksum_blocksize / 1024; append_printf(buffer, used, nbytes, "chksum_blocksize_kb = " I64T "\n", n);

   n = apr_time_sec(res->cache_expire);
   append_printf(buffer, used, nbytes, "n_cache = %d\n", res->n_cache);
   append_printf(buffer, used, nbytes, "cache_expire = %d\n", n);
   ...
   n = res->used_space[ALLOC_SOFT]/1024/1024; append_printf(buffer, used, nbytes, "#soft_used = " I64T " mb\n", n);
   n = res->used_space[ALLOC_HARD]/1024/1024; append_printf(buffer, used, nbytes, "#hard_used = " I64T " mb\n", n);
   n = res->used_space[ALLOC_SOFT]; append_printf(buffer, used, nbytes, "#soft_used = " I64T " b\n", n);
   n = res->used_space[ALLOC_HARD]; append_printf(buffer, used, nbytes, "#hard_used = " LU " b\n", n);

   append_printf(buffer, used, nbytes, "#n_allocations = " LU "\n", res->n_allocs);
   append_printf(buffer, used, nbytes, "#n_alias = " LU "\n", res->n_alias);
   i = append_printf(buffer, used, nbytes, "\n");
   return(i);
}
...
ibp_time_t trash_rescan(Resource_t *r, int tmode)
{
  ...
  while (osd_trash_iterator_next(iter, &id, &move_time, trash_id) == 0) {
     ...
     if ((nwipe % 100) == 0) atomic_inc(r->counter);
  }
  osd_destroy_iterator(iter);
...
//  apr_thread_mutex_unlock(r->mutex);
  ...
  while (osd_trash_iterator_next(iter, &id, &move_time, trash_id) == 0) {
     ...
     loop++;
     if ((loop % 100) == 0) atomic_inc(r->counter);
  }
  osd_destroy_iterator(iter);
...
void *resource_cleanup_thread(apr_thread_t *th, void *data)
{
  ...
  while (r->cleanup_shutdown == 0) {
     count = atomic_get(r->counter);
     if (count > 1073741824) atomic_set(r->counter, 0);
